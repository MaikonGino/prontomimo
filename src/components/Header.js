...
import {useSession, signOut} from 'next-auth/react'; // 1. Importa os hooks de sessão
import './Header.css';

export default function Header() {
    const {openCart, cartItems} = useCart();
    const router = useRouter();
    const {data: session, status} = useSession(); // 2. Pega o status da sessão (logado, deslogado, carregando)
    const totalItems = cartItems.reduce((count, item) => count + item.quantity, 0);

    const handleUserClick = () => {
        // Se estiver logado, vai para o dashboard. Se não, para a página de login.
            ...
            <div className="header">
                ...
                <div className="header-actions">
                    <button onClick={handleUserClick} className="action-button"
                            aria-label="Acessar área do cliente/administrador">
                        <Image src="/1000514721.png" alt="Ícone de usuário" width={40} height={40}
                               style={{borderRadius: '50%', backgroundColor: '#C9DFF2'}}/>
                    </button>
                    ...
                    {/* 3. Lógica Condicional: Se o usuário estiver autenticado, mostra o botão "Sair" */}
